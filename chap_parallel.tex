...
may nonetheless prove usefull to practitioners seeking to replicate these
...
independent of the number of SM's, and scalability to a large number of cores
...
reduction similar to that presented in the CUDA documentation and whitepapers,
even you suspect that a simpler reduction method would be faster and accurate enough.  
...
__device__ void dot(const float* __restrict__ a
	...
	// Do most of the dot product in a straightforward manner. (works!)
	while(i<n){
...
the version of cuda-gdb used does not properly support templates.   Device-side printf sometimes
...
Keeping all of the variables in on huge array of structures stored in global memory makes it
far easier to transfer them back to the CPU host, where printf is reliable.
\item If you only have a single problem instance to solve, we highly recommend using the CUBLAS library
and writing as little native CUDA code as possible.  Implementing the alignment stage took
far more development effort than the recognition stage, due not only to the CUDA's learning curve,
but due to the total absence of linear algebra libraries that provide an API within CUDA (rather than
