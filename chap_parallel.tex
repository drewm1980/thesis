\chapter{Parallelization of $\ell_1$-Minimization Based Face Recognition}
\label{chap:parallel}
...
architectures. Although $\ell_1$-minimization \eqref{eqn:l1min} is a convex
...
In addition, the accuracy of the solution given by \eqref{eqn:l1min_denoise} relies on the assumption
...
$\ell_1$-minimization objectives \eqref{eqn:l1min} and \eqref{eqn:l1min_denoise}, we 
also discuss how to efficiently accelerate the face alignment step
\eqref{eqn:l1min_alignment} on multi-core CPUs and GPUs.
...
\begin{figure}
\centering
%{\tiny \input{figures_ijcb/pipeline_simplified.tex}}
...
\eqref{eqn:l1min} \cite{YangA2010-ICIP} and analyze its complexity. Lagrange
...
iterative fashion.  For $\ell_1$-minimization \eqref{eqn:l1min}, the augmented Lagrange
function is defined as: 
\begin{equation} L_\mu(\xx,\blambda) = \|\xx\|_1 +
\blambda^T(\bb - A\xx) + \frac{\mu}{2} \| \bb-A\xx \|_2^2, 
\end{equation}
where $\mu > 0$ is a constant that penalizes infeasibility and $\blambda$ is a
vector of Lagrange multipliers.

In Lagrange Multiplier Theory \cite{BertsekasD2003}, if there exists a multiplier $\blambda^*$ that
...
\begin{equation}
\xx^* = \arg \min L_\mu(\xx,\blambda^*).
\label{eqn:optimal-ALM}
\end{equation}

In practice, the optimal values for the triplet $(\xx^*, \blambda^*, \mu)$ are all
unknown. Furthermore, it has been observed that solving
\eqref{eqn:optimal-ALM} with a large initial value of $\mu$ tends to lead to
...
iteratively update $\xx$ and $\blambda$:
\begin{equation}
\left \{
\begin{array}{lll}
\xx_{k+1} & = & \arg\min_{\xx} \, L_{\mu_k} (\xx,\blambda_k)\\
\blambda_{k+1} & = & \blambda_k + \mu_k (\bb - A\xx_{k+1}) \\
\end{array}
\right . ,
\label{eqn:alm}
\end{equation}
where $\mu_{k}\rightarrow \infty$ increases monotonically.
The iteration terminates when the estimates $(\xx_k, \blambda_k)$ converge.

Note that in the iterative procedure \eqref{eqn:alm}, the second
...
augmented Lagrangian $L_{\mu_k} (\xx,\blambda_k)$ compared to solving the original problem
\eqref{eqn:l1min} directly. In fact, this problem can be solved element-wise
...
Face alignment \eqref{eqn:l1min_alignment} estimates an image transformation
...
\eqref{eqn:l1min_alignment} is inefficient since it is a non-convex problem
...
\label{eqn:alignment-linearization}
\end{equation}
...
other words, eight instances of \eqref{eqn:alignment-linearization} are executed
...
in \eqref{eqn:l1min_denoise}, using Algorithm \ref{alg:alm_rec}.  The
