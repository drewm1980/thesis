...
underdetermined linear system $\bb=A\x$:
%
\begin{equation} \min \|\x\|_1\quad \mbox{ subj. to }\quad \bb = A\x.
...
\end{equation}
If the sparsest solutions for $\x$ and $\e$ are recovered, $\e$ provides a
means to compensate for pixels that are corrupted due to occlusion of some part of the query
image, and the dominant nonzero coefficients of $\x$ reveal the membership of
