...
\item Belongs to family of iterative shrinkage/thresholding methods
...
\frame{\frametitle{ALM Derivation}
\begin{itemize}
\item Seek a saddlepoint of $L_\mu(\x,\e,\blambda)$ 
...
\item Minimizing $L_{\mu} (\x,\e,\blambda_k)$ with respect to $\x$ and $\e$ simultaneously is costly.
\end{itemize}
}

\frame{\frametitle{ALM Derivation} 
\begin{itemize} 
\item Alternate between minimizing $L_{\mu} (\x,\e,\blambda_k)$
\begin{itemize} 
\item over $\x$ (with $\e$ fixed) and 
\item over $\e$ (with $\x$ fixed):
\end{itemize}
\end{itemize}
\begin{equation}
\left\{
...
\end{equation}
\begin{itemize}
\item The objective function is convex and alternation is guaranteed to coneverge to global optimum.
\end{itemize}
}

\frame{\frametitle{ALM Derivation} 
Soft-thresholding operator for $\x \in \Re^n, \alpha \in \Re$:
\begin{equation}
