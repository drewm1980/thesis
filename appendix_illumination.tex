...
%One of the main factors that complicates face recognition, and computer vision
%in general, is that two images of the same person's face can be very
%different, even if the pose is carefully controlled and all points on the
%person's face are visible in both images.  The most safe, reliable, and
%general assumption that can be made about the imaging process is that it is
%linear with respect to the intensity of different light sources.  The set of
%images of a given object is therefore a cone in the non-negative orthant of
%the pixel basis.  All recognition algorithms that rely on vision for training
%rely on images sampled from this cone.
